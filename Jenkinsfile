...
node(label: 'docker') {
    try {
        ...
        stage('test image') {
            sh("docker run --rm -e ENVIRONMENT=test -e COMPONENT_NAME=test-component -e ENGINEER_ENV='' aws-ecs-kinesis-log-forwarder:${GIT_TAG}-${BUILD_TIME} logstash --config.test_and_exit")
        }
